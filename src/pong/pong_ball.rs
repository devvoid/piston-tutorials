...
//Code taken from the aabb example.
struct Rect {
    x: f64,
    y: f64,
    w: f64,
...
fn main() {
    ...
    //Note that the paddle texture has been removed
    //The Rect struct means that all the variables needed to use Piston's built-in rectangle drawing functions are available
    let ball_texture = Texture::from_path(
        &mut window.factory,
    ...
    let mut font = Glyphs::new(
        assets.join("pong/coders_crux.ttf"), //Path to the font. I can't include a font in the tutorial files for copyright reasons, so you'll have to find your own
    ...
    while let Some(e) = window.next() {
        ...
        if let Some(_r) = e.render_args() {
                ...
                //Draw the ball
                image(&ball_texture, c.transform.trans(ball.x, ball.y), g);
